	...
	SocialTimeline.prototype = {
		...
		_createMarkup : function(outputOptions, num, afterLoad) {
			...
			for (x = num; x < this.entry.length; x++)
			{
                ...
                var time = this._relativeTime( published_parse );
                ...
                $authorandtime = $('<div />').addClass('author-and-time').html( '<strong>' + author + " · " + time + '</strong>');
                ...
                //$a = $('<a></a>').attr({href: this.entry[x].link, target: '_blank'});
                //$a.append($li);
					...
					$(outputOptions.$div).isotope({
					  ...
					  onLayout: function( $elems, instance ) {
                          ...
                          $elems.each(function(i){
                              var x = $(this).data('isotope-item-position').x;
                              var isIn=false;
                              for(var a=0;a<xPositions.length;++a){
                                  var v = xPositions[a];
                                  if(v == x) isIn = true;
                              }
                              if(!isIn) xPositions.push(x);
                          });

                          var itemsLeft = [];
                          $elems.each(function(i){
                              var x = $(this).data('isotope-item-position').x;
                              if(x == xPositions[0]) itemsLeft.push(this);
                          });

                          var itemsRight = [];
                          $elems.each(function(i){
                              var x = $(this).data('isotope-item-position').x;
                              if(x == xPositions[1]) itemsRight.push(this);
                          });

                          for(var a=0;a<itemsLeft.length;++a){
                              var $item = $(itemsLeft[a]);
                              var spanPointer = $('<span />');
                              spanPointer.addClass('left');
                              outputOptions.$dpSocialTimelineLine.append( spanPointer.css({top: ($item.data('isotope-item-position').y + 18)}) );
                          }

                          for(var a=0;a<itemsRight.length;++a){
                              $item = $(itemsRight[a]);
                              spanPointer = $('<span />');
                              spanPointer.addClass('right');
                              outputOptions.$dpSocialTimelineLine.append( spanPointer.css({top: ($item.data('isotope-item-position').y + 18)}) );
                          }
		...
		_relativeTime : function(time_value) {
			...
			if (delta < 60) { //60 sec
				...
				r = '' + (parseInt(delta / 3600)).toString() + ' Std.';
